...
async def process_directory(directory, provider, model_name, mode, tag, nummax=None):
    ...
    total_files = min(len(eligible_files),
                      nummax) if nummax else len(eligible_files)
...
async def main():
    parser = argparse.ArgumentParser(
        description="Process Markdown files in a directory and subdirectories using fnamer.")
    parser.add_argument(
        "directory", help="Directory containing Markdown files")
    parser.add_argument("--provider", choices=['huggingface', 'ollama',
                        'openai', 'google'], default='ollama', help="Choose the model provider")
    parser.add_argument(
        "--model", help="Specify the model name for the chosen provider")
    parser.add_argument("--mode", choices=['s', 'k'], default='s',
                        help="Analysis mode: 's' for summary, 'k' for classification")
    parser.add_argument("-n", "--nummax", type=int,
                        help="Maximum number of files to process")
    parser.add_argument("-tag", default="llama",
                        help="Tag to filter filenames (default: llama)")
    ...
    print(
        f"Total files processed this run: {min(args.nummax, results['total_files']) if args.nummax else results['total_files']}")
