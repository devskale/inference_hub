...
def loop_query_data():
    ...
    models_others = client.list_deployed_models(frameworks="all").get("text-generation", [])
    ...
    for m in tqdm(all_models):
        ...
        while run_text:
            try:
                run_text = False
                client.text_generation(m, "Hi.", max_new_tokens=1, timeout=10)
                text_available = True
            except Exception as e:
                if "Model requires a Pro subscription" in str(e):
                    pro_sub = True
        ...
        while run_chat:
            try:
                run_chat = False
                client.chat_completion(m, messages=[{'role': 'user', 'content': 'Hi.'}], max_tokens=1, timeout=10)
                chat_available = True
            except Exception as e:
                if "Model requires a Pro subscription" in str(e):
                    pro_sub = True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Fetch and update Hugging Face models data.')
    parser.add_argument('--query', type=str, default="", help='Search query for models')
    parser.add_argument('--filters', nargs='+', default=[], help='Filters for models')
    parser.add_argument('--use_cache', action='store_true', help='Use cached data if available')
    parser.add_argument('--update', action='store_true', help='Fetch and update models data')
